...
export default function EditListingPage() {
    ...
    const fetchListing = async () => {
      try {
        ...
        setForm({
          ...
          distance: listing.distance_from_campus || '',
          description: listing.description,
          amenities: Array.isArray(listing.amenities) ? listing.amenities : ['WiFi', 'Air Conditioning', 'Parking', 'Kitchen'],
        ...
        if (listing.photos && listing.photos.length > 0) {
          setImages(listing.photos.map(p => ({ preview: p.photo_url, id: p.id })));
        }
      } catch (err) {
        console.error(err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const existingUrls = images.filter(img => !img.file).map(img => img.preview);
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="card listing-form-card">
          ...
          <div style={{ borderRadius: 10, overflow: 'hidden', border: '1px solid var(--color-border)', height: 300 }}>
            <MapContainer
              center={form.latitude && form.longitude ? [form.latitude, form.longitude] : UTM_CENTER}
              zoom={14}
